...
import { formatBytes } from "./perdevicetraffichart";
...
export function PerDeviceTrafficTable({ data }: Props) {
  ...
  data.forEach(sample => {
    Object.keys(sample).forEach(key => {
      if (key === "date") return;
      if (!totals[key]) totals[key] = { inBytes: 0, outBytes: 0 };
      if (key.endsWith("-IN")) totals[key].inBytes += Number(sample[key] ?? 0);
      if (key.endsWith("-OUT")) totals[key].outBytes += Number(sample[key] ?? 0);
    });
  });
