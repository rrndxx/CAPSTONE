...
export function PerDeviceTrafficTable({ viewType = "All Devices", ipAddress }: Props) {
    const [trafficHistory, setTrafficHistory] = useState<DeviceTrafficSample[]>([]);
    const latestSample = usePerDeviceTraffic();

    // Append latest sample to history, keep last 60
    useEffect(() => {
        if (latestSample) {
    ...
    // Aggregate totals for each device
